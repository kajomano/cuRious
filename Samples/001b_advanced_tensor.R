# The previous script showed a very basic usage of tensors. If only life was
# that simple! This script shows the different levels (positions in memory) the
# tensors can inhibit, and how to move data between tensors.
library( cuRious )
...
# However, the tensor's data should not be accessed with the get.tensor accessor.
# This is because different tensors and R objects sharing the same memopry space
# could be modified unintentionally, without the lazy copy guarding them from
...
# An (intentional) unintentional modification:
not.really.copied.data <- tens.x$get.tensor
copied.data <- tens.x$pull()
...
# On level 1 the information is stored as single precision floating point values.
# Conversion is needed because most GPUs have much stronger single precisions
# capabilities than double, and neural networks do not really require double
# precision anyway. This, however, introduces a processing step between the two
# levels. Calling transform has no additional parameters how this conversion
# should be handled. This is because transform() is not intended as a frequently
# called operation, it is more to allocate memory space for copying operations.
# However, transfer(), the function that moves data between tensors, does have
# a few extra parameters. Lets create another tensor, and move the information
# to level 2:
...
# be plain R objects, which of course count as level 0). When transferring data
# between level 1 or 2 and level 0, the conversion can be done on multiple threads.
# Let's see how the number of threads modify the runtime of the conversion:
microbenchmark( transfer( tens.y, tens.x, threads = 1L ), times = 10 )
microbenchmark( transfer( tens.y, tens.x, threads = 4L ), times = 10 )
