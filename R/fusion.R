  ...
  private = list(
    ...
    .ep.tensor.add = function( tensor.ranged, tensor.name, output = FALSE ){
      if( !is.null( tensor.ranged ) ){
        tensor <- tensor.ranged$tensor

        # General endpoints and alert subscription
        private$.eps[[tensor.name]] <- tensor
        private$.subscribe( tensor, tensor.name )

        # Output endpoints for sever()
        if( output ){
          private$.eps.out[[tensor.name]] <- tensor
        }

        # Params
        private$.params[[ tensor.name ]]                    <- tensor
