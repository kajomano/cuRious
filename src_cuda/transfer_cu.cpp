...
void cuR_transfer_device_device_n_cu( float* src,
  ...
  if( stream ){
    cuR_transfer_device_device_n_cu_kern<float><<<numBlocks, blockSize, 0, *stream>>>( src, dst, dims[0], dims[1], csrc, cdst );
  }else{
    cuR_transfer_device_device_n_cu_kern<float><<<numBlocks, blockSize>>>( src, dst, dims[0], dims[1], csrc, cdst );
  }
}
